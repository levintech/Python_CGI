...
function runSearch( term ) {
    ...
    $.ajax({
        url: 'http://bfx3.aap.jhu.edu/hlam6/final/rev_comp.cgi',
        dataType: 'json',
        data: frmStr,
...
function processJSON( data ) {
    ...
    $.each( data.matches, function(i, item) {
        ...
        $('<td/>', { "Sequence Length" : item.len(out_seq) } ).appendTo('#' + this_row_id);
        
        // add the locus column
        $('<td/>', { "Transformed Sequence" : item.out_seq } ).appendTo('#' + this_row_id);
